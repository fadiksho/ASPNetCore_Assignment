...
/* WEBPACK VAR INJECTION */(function($) {__webpack_require__(/*! jquery-validation */ "./node_modules/jquery-validation/dist/jquery.validate.js");
...
var addNewPersonForm = $("#addNewPersonFormId");
var filterPersonListFormId = $("#filterPersonListFormId");
var personEditForm = $("#personEditFormId");
var person, seletedPersonTile; // delegate the event to capture new elements from ajax calls
...
$(document).on('click', '[edit-person]', function (e) {
  ...
  var personTileId = e.target.attributes["edit-person"].value;
  seletedPersonTile = $("#".concat(personTileId));
...
function getJsonDataFromPersonTile() {
  var personProperties = seletedPersonTile.find('[bindable-property]');
  person = {};
  personProperties.each(function (index) {
    var value = $.trim($(this).text()) || $(this).val();
...
function getJsonPersonFromEditForm() {
  var serializeArrayData = personEditForm.serializeArray();
  var updatedPerson = {};
  $.map(serializeArrayData, function (n, i) {
    var nameExpression = n['name'].split('.');
    var name = nameExpression[nameExpression.length - 1];
...
function populatePersonTileWithJsonData(data) {
  $.each(data, function (key, value) {
    var personField = $('[name=' + key + ']', seletedPersonTile);
    personField.text(value);
  });
}

addPersonSuccess = function addPersonSuccess(response) {
  addNewPersonForm.find("input[type=text]").val(""); // Todo: Add toast

  console.log(response);
};

addPersonFailure = function addPersonFailure(response) {
  // Todo: Add toast
  console.log(response);
};

updatePersonSuccess = function updatePersonSuccess(response) {
  toggleEditFormVisibility(false);
  var updatedPerson = getJsonPersonFromEditForm();
  populatePersonTileWithJsonData(updatedPerson);
  seletedPersonTile.show('slow'); // Todo: Add toast

  console.log(response);
};

updatePersonFailure = function updatePersonFailure(response) {
  // Todo: Add toast
  console.log(response);
};

deletePersonSuccess = function deletePersonSuccess(response) {
  if (response.success === true) {
    $("#person_row_".concat(response.id)).hide('slow'); // Todo: Add toast

    console.log(response);
  }
};

deletePersonFailure = function deletePersonFailure(response) {
  // Todo: Add toast
  console.log(response);
};
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js")))
